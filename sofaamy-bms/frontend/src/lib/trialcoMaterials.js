...
export const TRIALCO_GLASS_SHEET_M2 = 7.2
export const TRIALCO_GLASS_SHEET_INCREMENT = 0.5
...
export function trialcoMaterialRows(design) {
  ...
  const projectQty = Math.max(1, Number(design.qty || 1))
  ...
  const glassCutArea = glassQty * d.glassW * d.glassH / 1e6 * projectQty
  const glassPurchaseArea = d.frameW * d.frameH / 1e6 * projectQty
  const glassSheetQty = Math.ceil(
    (glassPurchaseArea / TRIALCO_GLASS_SHEET_M2) / TRIALCO_GLASS_SHEET_INCREMENT,
  ...
  const glassPricePerM2 = glass?.pricePerM2 ?? 120
  const glassMeta = {
    code:selectedGlass,
    unit:`${TRIALCO_GLASS_SHEET_M2}m² sheet`,
    unitPrice:glassPricePerM2 * TRIALCO_GLASS_SHEET_M2,
    source:'Glass catalogue',
  }
  ...
  const baseRows = [
    ...
    row('trialco-net', 'Trialco net', barsFor(netCuts, projectQty), TRIALCO_MATERIAL_PRICES.net.unitPrice, TRIALCO_MATERIAL_PRICES.net,
      `${roundQty(netLength * projectQty / 1000)} m required · ${netQty} net × perimeter`),
    ...
    row('trialco-kit', 'Trialco kits', projectQty, TRIALCO_MATERIAL_PRICES.kit.unitPrice, TRIALCO_MATERIAL_PRICES.kit,
      '1 set per bay'),
    row('glass', `Glass — ${glass?.label || selectedGlass}`, glassSheetQty, glassMeta.unitPrice, glassMeta,
      `${roundQty(glassPurchaseArea)} m² frame-area basis ÷ ${TRIALCO_GLASS_SHEET_M2} m²/sheet = ${glassSheetQty} sheets (rounded to 0.5); cut estimate ${roundQty(glassCutArea)} m² · 2 panels × Glass W × Glass H`),
    row('0404-corners', '0404 corners', 4 * (1 + leafQty) * projectQty, TRIALCO_MATERIAL_PRICES.corner.unitPrice, TRIALCO_MATERIAL_PRICES.corner,
      '4 outer-frame corners + 4 corners per leaf'),
    row('trialco-rollers', 'Trialco rollers', 2 * leafQty * projectQty, TRIALCO_MATERIAL_PRICES.rollers.unitPrice, TRIALCO_MATERIAL_PRICES.rollers,
      '2 rollers per leaf'),
    row('metal-locks', 'Metal locks', projectQty, TRIALCO_MATERIAL_PRICES.locks.unitPrice, TRIALCO_MATERIAL_PRICES.locks,
      '1 lock per bay'),
    ...
    row('net-fibre', 'Net fibre', netArea * projectQty, TRIALCO_MATERIAL_PRICES.netFibre.unitPrice, TRIALCO_MATERIAL_PRICES.netFibre,
      `${roundQty(netArea * projectQty)} m² · ${netQty} net × Net W × Net H`),
  ...
  return {
    ...
    glassCutArea:roundQty(glassCutArea), glassPurchaseArea:roundQty(glassPurchaseArea),
    glassSheetQty, glassSheetAreaM2:TRIALCO_GLASS_SHEET_M2,
    priceSource:'Trialco internal costing sheet rates · glass catalogue sheet rate',
  }
}
